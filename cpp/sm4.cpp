...
int sm4_crypt_ecb_pkcs7(sm4_context *ctx,
                        ...
                        unsigned char *output)
{
    if (ctx->padding_type != SM4_PADDING_PKCS7)
    {
    ...
    unsigned char *input_new = NULL;
    unsigned char *p = NULL;
    int input_new_len = length + 16;
    int diff = 16 - length % 16;
    if (mode == SM4_ENCRYPT)
    {
        if (diff > 0)
        {
            input_new = (unsigned char *)malloc(input_new_len);
            if (input_new == NULL)
            {
                free(input_new);
                input_new = NULL;
            ...
            p = input_new;
            memset(input_new, 0, input_new_len);
            memcpy(input_new, input, length);
            int i = 0;
            for (; i < diff; i++)
            {
                p[length + i] = diff;
    ...
    if (mode == SM4_DECRYPT)
    {
        int p = pOut[out_len - 1];
        memset(pOut + out_len - p, 0, p);
        out_len -= p;
    }
    return out_len;
}